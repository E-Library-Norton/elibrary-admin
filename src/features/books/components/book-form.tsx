...
const MAX_EXTRA_PDFS = 4; // additional PDFs beyond the primary
...
const formSchema = z.object({
  ...
  pdfUrl: z
    ...
    .optional()
    .refine(
      (files) => !files || files.length === 0 || files?.[0]?.size <= MAX_PDF_SIZE,
  ...
  // Additional PDF files (up to MAX_EXTRA_PDFS)
  pdfUrls: z
    .any()
    .optional()
    .refine(
      (files) => !files || files.length === 0 || Array.from(files as FileList).every((f: File) => f.size <= MAX_PDF_SIZE),
      'Each PDF must be ≤ 50MB.'
    )
    .refine(
      (files) => !files || files.length === 0 || Array.from(files as FileList).every((f: File) => f.type === 'application/pdf'),
      'Only PDF files are accepted.'
    )
    .refine(
      (files) => !files || files.length <= MAX_EXTRA_PDFS,
      `You can upload up to ${MAX_EXTRA_PDFS} additional PDFs.`
    ),
  ...
  videoUrl: z
    ...
    .optional()
    .refine(
      (files) => !files || files.length === 0 || files?.[0]?.size <= MAX_VIDEO_SIZE,
  ...
  audioUrl: z
    ...
    .optional()
    .refine(
      (files) => !files || files.length === 0 || files?.[0]?.size <= MAX_AUDIO_SIZE,
...
export default function BookForm({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      typeId: initialData?.MaterialType?.id ? String(initialData.MaterialType.id) : '',
  ...
  async function onSubmit(values: FormValues) {
    try {
      ...
      const pendingEditorNames = editorInput.trim()
        ? editorInput.split(',').map((n) => n.trim()).filter(Boolean)
        : [];
      ...
      const pendingPublisherNames = publisherInput.trim()
        ? publisherInput.split(',').map((n) => n.trim()).filter(Boolean)
        : [];
      ...
      const payload = {
        ...
        ...(finalPublisherNames.length > 0 && { publisherNames: finalPublisherNames }),
    ...
    <Card className='mx-auto w-full'>
      ...
      <CardContent>
        <Form form={form} onSubmit={form.handleSubmit(onSubmit)} className='space-y-8'>
          ...
          {initialData && (initialData.coverUrl || initialData.pdfUrl || initialData.pdfUrls?.length || initialData.videoUrl || initialData.audioUrl) && (
            <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4'>
              ...
              {initialData.videoUrl && (
                <div className='space-y-2'>
                  ...
                  <p className='text-xs text-muted-foreground'>Upload a new video to replace</p>
                </div>
              )}
              {initialData.audioUrl && (
                <div className='space-y-2'>
                  ...
                  <p className='text-xs text-muted-foreground'>Upload a new audio to replace</p>
                </div>
              )}
            </div>
          )}
              ...
              >
                {authorNames.map((name, idx) => (
                  ...
                  >
                    ...
                    <button
                      type='button'
                      onClick={() => setAuthorNames((prev) => prev.filter((_, i) => i !== idx))}
                ...
                <input
                  ...
                  placeholder={authorNames.length === 0 ? 'Type author name, press Enter or comma to add…' : ''}
              ...
              >
                {editorNames.map((name, idx) => (
                  ...
                  >
                    ...
                    <button
                      type='button'
                      onClick={() => setEditorNames((prev) => prev.filter((_, i) => i !== idx))}
                      className='hover:text-destructive transition-colors leading-none'
                      aria-label={`Remove ${name}`}
                    >
                      <X className='h-3 w-3' />
                ...
                <input
                  ...
                  placeholder={editorNames.length === 0 ? 'Type editor name, press Enter or comma to add…' : ''}
                  className='flex-1 min-w-[8rem] outline-none bg-transparent placeholder:text-muted-foreground text-sm'
                />
              </div>
              <p className='text-xs text-muted-foreground'>
              ...
              >
                {publisherNames.map((name, idx) => (
                  ...
                  >
                    ...
                    <button
                      type='button'
                      onClick={() => setPublisherNames((prev) => prev.filter((_, i) => i !== idx))}
                      className='hover:text-destructive transition-colors leading-none'
                      aria-label={`Remove ${name}`}
                    >
                      <X className='h-3 w-3' />
                ...
                <input
                  ...
                  placeholder={publisherNames.length === 0 ? 'Type publisher name, press Enter or comma to add…' : ''}
                  className='flex-1 min-w-[8rem] outline-none bg-transparent placeholder:text-muted-foreground text-sm'
                />
              </div>
              <p className='text-xs text-muted-foreground'>
